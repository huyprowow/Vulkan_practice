...
private:
    ...
    void pickPhysicalDevice()
    {
        ...
        //     auto deviceProperties = device.getProperties(); // thong tin card, pb vk sp,...
        //     auto deviceFeatures = device.getFeatures();     // thong tin tinh nang card ho tro
        //     uint32_t score = 0;
    ...
    {
        // note: Các trình điều khiển hiện có chỉ cho phép bạn tạo một số lượng nhỏ hàng đợi cho mỗi họ hàng đợi, không cần nhiều hơn vì có thể tạo tất cả các bộ đệm lệnh trên nhiều luồng, sau đó gửi tất cả chúng cùng một lúc trên luồng chính chỉ với một lệnh gọi
        ...
        graphicsQueue = vk::raii::Queue(device, graphicsIndex, 0);// hang doi lenh do hoa
    }
